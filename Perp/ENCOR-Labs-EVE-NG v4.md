...
**Apply on DIST1 and DIST2** (downlinks toward ACC — these ports must never receive a superior BPDU, because nothing below DIST should ever try to be root):
```
! On DIST1 — ports facing ACC1 and ACC2:
interface GigabitEthernet0/2
 spanning-tree guard root
interface GigabitEthernet0/3
 spanning-tree guard root
!
! On DIST2 — ports facing ACC1 and ACC2:
interface GigabitEthernet0/2
 spanning-tree guard root
interface GigabitEthernet0/3
 spanning-tree guard root
```
*(Use `show cdp neighbors` on each DIST to confirm which port goes to which ACC — numbering depends on your EVE-NG link order.)*

**Critical: where Root Guard goes and why.** Root Guard goes on ports facing **devices that should never become root** — i.e. on a root/distribution switch, facing access switches below it. It does **NOT** go on access→distribution uplinks, because those uplinks are *supposed* to receive superior BPDUs from the legitimate root. If you put Root Guard on ACC uplinks, every uplink will go to `root-inconsistent` immediately, the access switch loses its path to root, and STP falls apart. Common mistake — don't make it.
...
2. Test Root Guard on DIST. From ACC1, temporarily try to become root by lowering ACC1's priority below DIST1's: `spanning-tree vlan 10 priority 0`. ACC1 sends a superior BPDU up to DIST. With Root Guard on DIST's downlinks toward ACC, **DIST1's port facing ACC1 goes `root-inconsistent`** (not ACC1's uplink — the protection lives on the upstream side):
   ```
   ! Run on DIST1, not on ACC1:
   show spanning-tree inconsistentports
   ```
   Expected: DIST1's Gi0/2 (port toward ACC1) listed in root-inconsistent for VLAN 10. ACC1 can't actually steal root because DIST is filtering ACC1's superior BPDU. Undo with `no spanning-tree vlan 10 priority` on ACC1 — DIST's port recovers automatically in ~15s.
