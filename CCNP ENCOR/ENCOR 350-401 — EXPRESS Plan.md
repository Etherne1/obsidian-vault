...
**Trackers (unchanged):** `ENCOR-Progress-Tracker.csv`, `ENCOR-Progress-Tracker-2.md`.
...
**Video spine:** same as main plan — CBT Nuggets first (concepts), INE second (protocol depth). Labs from GNS3vault archive.
**Time budget:** ~35h theory (CBT+INE+OCG) + ~20h labs = ~55h.

---

## Block 1 — Architecture & Design (~5h theory, no dedicated lab)
...
| Architecture (added) | build a 3-tier baseline topology from scratch, no starter config | 2h |
| Architecture (added) | NSF/SSO failover demo or VSS/StackWise config review | 2h |
